...
class aAgent(Agent):
        ...
        if self.first_move:
            ...
            if self.searchTree.root.childNodes != None:
                for childNode in self.searchTree.root.childNodes:
                    if my_pos == childNode.my_pos and adv_pos == childNode.adv_pos and np.array_equiv(chess_board,
                                                                                                      childNode.chess_board):
            ...
            if (not foundChild):
                self.searchTree.root = Node(chess_board, my_pos, adv_pos, True, None,
                                            None)  # Should check garbage collection here too.
            # Run MCTS:
    ...
    def Monte_Carlo_Tree_Search(self, max_step, first_move):
        ...
        return max(self.searchTree.root.childNodes, key=lambda x: aAgent.nodeValue(
            x))  # We assume we'll always have a child node to choose. To be safe we could put an if statement here.
...
class SearchTree:
    ...
    def selectHelper(node):
        if (
                node.unusedMoveSet == None or not node.unusedMoveSet.isEmpty()):  # This is equivalent to isFullyExpanded == False. There is no boolean isFullyExpanded, its implied by this statement.
            return node
        else:
    ...
    def expand(leaf, max_step):
        ...
        if (terminal or leaf.totalPlays == 0):
            ...
            child = leaf.createChildNode(
                max_step)  # Generates a child node based on a random move within the max_step limits.
            return child
    ...
    def simulate(node, max_step):
        ...
        while (not gameOver):
            ...
            if (
            not node.my_turn):  # i.e. Our agent is not the starting_player. Thus we want to check if a game is over every time the advesary moves.
                gameOver, strt_player_score, secd_player_score = SearchTree.isTerminalState(chess_board_copy,
                                                                                            starting_player_pos,
                                                                                            secondary_player_pos)
                if (gameOver): break
            ...
            Node.set_barrier(chess_board_copy, r, c, dir)
            if (
            node.my_turn):  # i.e. our agent is not the secondary_player. Thus we want to check if a game is over every time the advesary moves.
                gameOver, strt_player_score, secd_player_score = SearchTree.isTerminalState(chess_board_copy,
                                                                                            starting_player_pos,
                                                                                            secondary_player_pos)
                ...
                if cur_step != max_step:
                    for dir in range(0, 4):
                        ...
                        if (not self.chess_board[row, column, dir] and tuple(new_pos) not in visited
                                and not np.array_equal(new_pos, self.my_pos) and not np.array_equal(new_pos,
                                                                                                    self.adv_pos)):
